...
describe('Authentication', function() {
  ...
  it('Should return access token with expiration > now()', async function() {
    let accessToken = await HelpScoutClient.getAccessToken();
...
describe('List', function() {
  it('Can List Mailboxes', async function() {
    let mailboxes = await HelpScoutClient.list('mailboxes');
...
describe('Get', function() {
  it('Get Customer', async function() {
    let foundCustomer = await HelpScoutClient.get('customers', customerId);
    assert(foundCustomer);
  });

  it('Get Conversation', async function() {
    let foundConversation = await HelpScoutClient.get('conversations', conversationId);
    assert(foundConversation);
  });

  it('Get Mailbox', async function() {
    let mailbox = await HelpScoutClient.get('mailboxes', mailboxId);
    assert(mailbox);
  });
});
